...
Edit `level` in `assets/lang/en.json` **and** `assets/lang/es.json` — they must
match, and `flutter test` fails if they drift. Never change `id`, `row` or `col`
while doing it.
