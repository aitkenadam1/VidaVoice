...
class KokoroTtsService {
  ...
  /// Download size in bytes (126.2 MB at pack time). Shown in Settings.
  static const int modelDownloadBytes = 132314368;
  ...
  /// two playbacks share (or race on) one file.
  Future<bool> speak(
    String text, {
    ...
    try {
      ...
      result = await replyPort.first as _KokoroSpeakResult;
    } catch (_) {
    ...
    while (!input.isEOS) {
      ...
      switch (typeFlag) {
        ...
        case 0x00: // '\0': regular file (pre-POSIX tars)
          ...
          await _skipPadding(input, size);
        default: // symlinks, pax headers, devices: skip the data
          await _skipFully(input, size);
      }
    }
  } finally {
